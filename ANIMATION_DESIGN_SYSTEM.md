...
### O. Emitter Nozzle Geometric Pinning & Anti-Bleed Containment (Nozul/Ağız Ekseni Kilitlenmesi ve Kart Sınırı Bütünlüğü — Arbok Poison Vapor Dersi)
- **Nozul Ekseni Sapması ve Havada Boşluktan Püskürme Yasağı:**
  Bir Pokémon'un ağzından, namlusundan veya organından çıkan sıvı, gaz, alev veya enerji akıntılarında (Arbok *Poison Vapor*, Horsea *Ink Jet*, Blastoise *Hydro Pump* vb.); püskürme akıntısının (jet / plume) başlangıç noktası aktörün gövdesinin altından veya havada bağımsız bir boşluktan başlayamaz. Püskürme akıntısının koordinat merkezi doğrudan aktörün anatomik ağız açıklığına pikseli pikseline kenetlenmelidir (`top: 22%`, `transform-origin: 50% 0%`, üst nozul kilitlenmesi).
- **Lunge ile Senkronize Akıntı Fiziği:**
  Aktör öne veya hedefe doğru atıldığında (`lunge`), püskürme akıntısı da aktörün ağız açıklığıyla eşzamanlı olarak hedefin üzerine fışkırmalı; aktör geriye çekildiğinde miazma/akıntı sahneye yayılarak aktörden kopmalıdır.
- **Tavizsiz Kart İçi Hapsedilme (Anti-Bleed Containment):**
  Püskürtülen akıntı konteyneri hedef kartın fiziksel sınırları içinde tutulmalı (`.card-fx-block-overlay rounded-xl overflow-hidden`), gaz veya sıvının alt kuyruğu hedef kartın alt kenarından dışarı (arena zeminine veya deste alanına) taşmamalıdır. Üst kenarda ise aktörün kafa kubbesinin kesilmemesi için güvenli marjin (+11px) korunmalıdır.

### P. Bench Multi-Target Cinematic Quality Standard (Bench Hasarı Sinematik Kalite Standardı — Minyatür 4-5 Katmanlı Reaksiyon)
- **Tekdüze Renk Lekesi ve Statik Çember Yasağı:**
  Alan etkili (AOE) veya süpürme hasarı içeren çoklu hedef saldırılarında (Arbok *Poison Vapor*, Articuno *Blizzard* vb.); yedek (bench) kartları üzerindeki efektler aceleye getirilmiş tekdüze renk lekelerinden veya basit içi boş çemberlerden ibaret olamaz. Bu yüzeysel yaklaşım bench hasarını önemsiz bir yan hata gibi hissettirir.
- **Minyatür 5 Katmanlı Kimyasal/Fiziksel Reaksiyon Mimarisi:**
  Küçük boyuttaki yedek kartlar (`size="sm"`, `110px × 151px`) üzerinde de aktif karttaki vuruş kadar özenli, minyatür 4-5 katmanlı bir reaksiyon orkestre edilmelidir:
  1. *[Katman 1] Taban Aurası:* Kart zemininde yayılan elementel zemin aurası (`gbaVaporBenchAura`),
  2. *[Katman 2] Dönen Miazma/Duman Pufu:* Organik çok loblu dönen duman pufu (`gbaVaporBenchPuff`),
  3. *[Katman 3] Mikro Kavitasyon/Köpük Kabarcıkları:* Patlayan 4 aşamalı asit veya buz kabarcıkları (`gbaVaporBenchBubble`),
  4. *[Katman 4] Süzülen Aerosol Zerrecikleri:* Havada asılı kalan ve yavaşça dağılan mikro zerreler (`gbaVaporBenchFlecks`),
  5. *[Katman 5] Hasar Temas Flaşı:* Darbe anında hedef kartı anlık aydınlatan hasar doygunluğu flaşı (`gbaVaporBenchDamageFlash`).
- **Kinematik Süreklilik ve Senkronizasyon:**
  Bench animasyonları aniden başlayıp kesilemez (`poison_vapor_bench 1320ms` örneğindeki gibi); kademeli bir yayılma ve sönümlenme zarfı taşımalı, `GameBoard.tsx` kademeli hasar gecikmeleri (+240ms, +350ms, +460ms) ile milisaniyesine senkronize olmalıdır.
...
2. **Card Boundary Containment & Anti-Bleed Protocol (Kart Çerçevesi İçi Kırpma & Taşma Önleme):**
   ...
   - **Nozul ve Ağız Ekseni Kilitlenmesi (Maw/Nozzle Origin Pinning):**
     - Bir Pokémon'un ağzından, namlusundan veya organından çıkan jet, alev, su veya gaz püskürmelerinde (Arbok *Poison Vapor*, Horsea *Ink Jet* vb.); püskürme akıntısının başlangıç orijini aktörün anatomik ağız açıklığına pikseli pikseline kenetlenmelidir (`transform-origin: 50% 0%`, üst nozul ekseni, `top: 22%` kilitlenmesi).
     - Akıntı aktörün altından veya havada bağımsız bir boşluktan fışkıramaz; akıntı konteyneri hedef kartın fiziksel sınırları içinde tutulmalı (`overflow-hidden rounded-xl`), kartın alt sınırının dışına taşma (boundary bleed) yaşanmamalıdır.
...
3. **Multi-Target & Bench Slot Awareness (Slot-Bilinçli FX Degradasyonu, Okunabilirlik Peçesi & Bench Kalite Standardı):**
   - Special moves (e.g., Gengar *Dark Mind*, Ninetales *Lure*, Arbok *Stare*, Poison Vapor) can target either the Active card or Bench slots (`slot === 'bench'`, `benchIndex`).
   - Coordinate logic must gracefully support bench transposition without visual clipping.
   - **Bench Hasarı Sinematik Kalite Standardı (Bench Cinematic Quality Standard):**
     - Çok hedefleri kapsayan süpürme saldırılarında (Poison Vapor, Blizzard vb.), bench kartları üzerindeki efektler aceleye getirilmiş tekdüze renk lekelerinden veya statik çemberlerden ibaret olamaz.
     - Küçük boyuttaki bench kartları (`size="sm"`, `110px × 151px`) üzerinde de minyatür 4-5 katmanlı bir kimyasal/fiziksel reaksiyon yaşanmalıdır (zemin aurası, dönen duman pufu, mikro köpük kabarcıkları, süzülen aerosol zerrecikleri ve hasar flaşı).
     - Süreler ani başlayıp bitmemeli (`poison_vapor_bench 1320ms` örneği gibi), kademeli bir yayılma ve sönümlenme eğrisi taşımalıdır.
...
4. **The Absolute-in-Flex Static Coordinate Anchor Invariant (Flexbox İçi `position: absolute` Eş Eksenli Kilitlenme Kuralı):**
   - **Tasarım / Motor Tuzağı (The Static Zero Trap):** CSS Flexbox spesifikasyonuna göre (`display: flex; items-center; justify-center;`), bir alt elemana `position: absolute` verildiğinde, bu eleman normal flex düzen akışından (layout flow) tamamen çıkarılır.
   - Eğer bu mutlak elemana açıkça koordinat (`top`, `left`, `bottom`, `right`) tanımlanmazsa; tarayıcı elemanın statik konumunu en yakın konumlandırılmış üst kapsayıcının sol-üst köşesine yani `(0, 0)` noktasına kilitler.
   - **Felaket Anı (Translate Override Çarpışması):** İlgili elemana ait CSS `@keyframes` kuralı `transform: translate(-50%, -50%) scale(...)` içeriyorsa; tarayıcı bu `-50%` ötelenmeyi kartın merkezine göre değil, elemanın statik `(0, 0)` koordinatına göre hesaplar. Sonuç olarak eleman (örneğin 160×120px'lik bir biyoelektrik plazma küresi), kendi yarıçapı kadar (`-80px` X, `-60px` Y) **hedef kartın sol-üst sınırlarının dışına fırlar ve boşlukta patlar!** (Bkz. Grimer *Sticky Hands* L16338 plazma küresi hatası).
   - **Kesin Kural (Coaxial Anchor Invariant):**
     1. Keyframe'inde `translate(-50%, -50%)` (veya `translateX(-50%)`) kullanan İSTİSNASIZ TÜM mutlak elemanlar, JSX seviyesinde açıkça `style={{ left: '50%', top: '50%' }}` (veya yatay merkezleme için `left: '50%'`) koordinat kilitlenmesine sahip olmak ZORUNDADIR.
     2. Ebeveyn kapsayıcının `flex items-center justify-center` sınıfına sahip olması, mutlak pozisyonlu bir elemanı asla otomatik olarak merkezlemez; flex sınıflarına güvenilerek koordinat tanımları ihmal edilemez.
     3. Zemin auralarında (`absolute bottom-X`), yatay eksen ya `w-full flex justify-center` ile tam kart genişliğine yayılmalı ya da `left: 50%; transform: translateX(-50%)` ile dikey simetri eksenine kilitlenmelidir.
     4. Tam kartı örten partikül ve buhar katmanlarında `inset-0` veya `w-full h-full` açıkça belirtilerek statik sıfır sapmaları önlenmelidir.
5. **Preview Stage vs. In-Game Active & Bench Card Dimension, Parity & CSS Hygiene Discipline (Önizleme Sahnesi & Oyun İçi Kart Parite ve CSS Hijyen İlkesi):**
   - **Boyutsal Ayrışma Riski (Dimension Desynchronization):** Önizleme (showcase) HTML sayfalarında (`preview_*.html`), test kartı kapsayıcılarının (`.card-stage`) rastgele veya tahmini boyutlarla (örneğin `180px × 252px`, `184px × 256px`, `110px × 152px`) inşa edilmesi; gerçek oyun içi aktif ve bench kart boyutlarıyla (`CardView.tsx` `size="active"` = `184px × 253px`, `size="sm"` = `110px × 151px`, her ikisi de `aspect-ratio: 600 / 825`) mikro-ölçek uyuşmazlığı yaratır.
   - **Soyut Sahne Körlüğü (Blind Prototyping):** Efektlerin boş siyah/koyu dikdörtgenler üzerinde test edilmesi; animasyonun hedef kartın anatomik bileşenleriyle (Ken Sugimori illüstrasyon penceresi, HP göstergesi, saldırı metin kutusu, zayıflık pencereleri) kurduğu görsel ilişkiyi gizler. Bu durum, önizleme tahtasında "normal ve merkezde" algılanan bir efektin, gerçek oyun tahtasında kartın resim kutusunu ıskalamasına veya dışa taşmasına yol açar.
   - **Parite Standartları:**
     1. *Piksel-Piksele Boyut Eşitliği:* 
        ...
        - Tüm bağımsız önizleme sahneleri oyun içi masaüstü `GameBoard.tsx` ve `CardView.tsx` CSS ölçülerini ve en-boy oranını birebir kullanmalıdır.
     2. *Otantik Durum Şeridi Paritesi (Status Strip Parity):* Önizleme kartları yalnızca boş bir çerçeve değil, `CardView.tsx`'teki gibi üst durum şeridini (HP sayısal değeri ve segmentli yeşil can pips'leri) barındırmalıdır. Bu sayede efektlerin durum çubuğunun okunabilirliğini engelleyip engellemediği doğrudan test edilebilir.
     3. *Otantik Kart Altlığı Gereksinimi & Çift Modlu Doğrulama (Real Card Mode vs. Dark Canvas):* Önizleme sayfaları, test edilen saldırının hedef kart tipine uygun otantik Base Set / Jungle / Fossil kart görsellerini (`cards/53.jpg` Magnemite, `cards/48.jpg` Grimer, `cards/13.jpg` Muk, `cards/14.jpg` Weezing, `cards/51.jpg` Koffing vb.) alt katman (`underlay`) olarak barındırmalıdır. Geliştiricinin hem pürüzsüz kart illüstrasyonunu hem de minimalist soyut koyu aşamayı kıyaslayabilmesi için dinamik bir "Mod Değiştirici" (Toggle Mode) düğmesi (`#btnMode`) sunulmalıdır.
     4. *CSS Sözdizimi Hijyeni & Bağımsız Keyframe Kuralı (Root-Level Keyframes Invariant):*
     ...
     5. *Dayanıklı Varlık Yolları (Resilient Path Protocol):* `candidatePaths` dizisi ve `onerror` zincirlemesi ile hem yerel `file:///` hem de dev sunucu ortamlarında kayıpsız görsel yükleme garantilenmelidir (Kural 14).
     6. *Stil ve Kod Özdeşliği (Zero-Deviation CSS/JSX):* Önizleme HTML dosyasındaki stiller ve DOM yapısı ile `BattleFXOverlay.tsx` içindeki üretim JSX yapısı arasında sıfır fark bulunmalıdır; önizlemede çalışan bir koordinat kilitlenmesi üretim kodundan esirgenemez.
...
- **C. Grimer Sticky Hands: Biyoelektrik Arklar, Viskoelastik Lifler ve Zamanlama Dengelemesi (`grimer_sticky_hands`, 1650ms):**
  ...
  - **Sorun 3 (Statik Sıfır Koordinat Kayması & Rakip Kart Dışına Taşma):** Önizleme tahtasında merkezde oynatılan yeşil plazmatik enerji küresi (Katman 5) ve amorf mukus lifleri, oyun içinde aktif rakip kartı (Magnemite) üzerinde kartın sol-üst köşesinden dışarı fırlayarak arena zemininde patlıyordu.
  - **Mimari Çözüm:**
    ...
    4. *Flexbox Eş Eksenli Kilitlenme (L16149, L16314, L16338):* `@keyframes gbaStickyParalysisRing` ve `gbaStickyWebPulse` içindeki `translate(-50%, -50%)` kuralının statik sıfır `(0, 0)` koordinatından fırlamasını önlemek adına; Katman 1 (zemin), Katman 4 (mukus lifleri) ve Katman 5 (plazma deşarj küresi) elemanlarına açıkça `style={{ left: '50%', top: '50%' }}` koordinat kilitlenmesi uygulandı (§6.4).
    5. *Dengeli Ölçek Kalibrasyonu (Kademeli ~%12–%14 Dengeleme):* Grimer ve Muk'ın sahnede kartı boğmadan heybetlerini koruması adına; Sticky Hands aktör taban boyutu 140px'ten 122px'e (whiff: 86px), plazma küresi 138×104px'e, mukus ağ lifleri 132×96px'e çekildi. Muk Deluge aktörü 144px'ten 126px'e, Nasty Goo çekirdeği 160px'ten 140px'e, Minimize göleti 140px'ten 122px'e kalibre edildi. Keyframe tepe scale çarpanları da (~%10–%12) dengelenerek tepe vuruş anında 184px'lik kart sınırları içinde güvenli nefes payı bırakması sağlandı (§3.F).
...
- **E. Önizleme Sahnesi Parite ve Varlık Dayanıklılık Standartları (`preview_grimer_muk.html` Dersi):**
  - **Sorun:** Önizleme sayfasının soyut `180px × 252px` siyah kutu kullanması, oyun içindeki `184px × 253px` (`CardView.tsx` aktif boyutları) oran farkını gizliyor; gerçek kart illüstrasyonu altlığı olmadan animasyonun hedef anatomisine (art box, HP, saldırı kutusu) göre konum doğruluğu test edilemiyordu.
  - **Mimari Çözüm:**
    1. *Kart Sahnesi Boyut Paritesi:* Önizleme `.card-stage` kapsayıcısı pikseli pikseline masaüstü aktif kart standardı olan `184px × 253px; aspect-ratio: 600 / 825; border-radius: 12px` ölçülerine eşitlendi.
    2. *Otantik Kart Altlıkları:* Hedef saldırılar için Base Set Magnemite (`cards/53.jpg`), savunma/kaçınma (Minimize) için Grimer (`cards/48.jpg`) altlıkları eklendi.
    3. *Dinamik Mod Seçici (Toggle Mode):* Geliştiricinin hem otantik TCG kartı üzerinde hem de minimalist şeffaf sahnede inceleme yapabilmesi için `🃏 Mod: Gerçek TCG Kartı / Sade Aşama` toggle düğmesi entegre edildi.
    4. *Dayanıklı Yükleme & Sahne Temizliği:* Kural 12 uyumlu `candidatePaths` döngüsüyle hem yerel dosya sisteminden (`file:///`) hem dev sunucudan (`http://localhost:5173/`) hatasız yükleme garantilendi; saldırı başlatıldığında sahne DOM elemanları `cleanStage()` ile sızıntısız sıfırlandı.
...
### 10.9 Vaka Çalışması: Dark Arbok Kobra Salınımı, Hipnotik Bakış & Susturma Mührü (`cobra_stare`, 1750ms)

**Kaynak:** `BattleFXOverlay.tsx` L633 (dispatch `cobra_stare`), L1380 (`getFXDuration` = 1750), L14853–15030 · `index.css` L7155–7250 · `preview_dark_arbok.html` · Görsel Varlıklar: `dark_arbok_stare.png` (Tier 1 Apex `116px × 108px`, whiff: `84px × 78px`).

- **A. Çift Görsel ve Opasite Kilitlenme Hatası (Duplicate DOM Opacity Bug - L14873):**
  - **Sorun:** Eski kodda `dark_arbok_stare.png` iki ayrı `<div>` içine konulmuştu: Girişte Beat 1 (`gbaStareMaterialize ... forwards`) `0.85s`'de `%100` opaklığa ulaşıp kilitleniyordu; çıkışta ise Beat 3 (`gbaStareDissolveOut`) ikinci bir kopya açıp erimeye çalışıyordu. Ancak Beat 1 arkada `%100` opak kaldığı için görsel eriyemiyor, son yarım saniye (~450ms) boyunca hareketsiz, donuk ve yapay bir fotoğraf gibi ekranda asılı kaldıktan sonra bileşen unmount edilince aniden yok oluyordu.
  - **Mimari Çözüm:** İki ayrı görsel yerine tek bir otantik `<img>` elemanı kullanıldı. Kobra salınımı, lunge gerilimi ve gölgeye çekilme tek bir kesintisiz keyframe (`gbaArbokCobraMotion 1.75s`) içine yedirildi; %0 idle freeze ile akıcı sönümlenme sağlandı.
- **B. Kesintisiz Kobra Momentum & Göğüs Deseni Işıması (Continuous Momentum & Chest Eyes Pulse):**
  - **Sorun:** Kobra statik bir damga gibi duruyor; yılan anatomisine özgü yaylanma ve saldırı atılışı bulunmuyordu.
  - **Mimari Çözüm:** Dört fazlı kobra fiziği inşa edildi:
    1. *Zeminden Yükseliş & Salınım (0.00s–0.35s):* `translateY(32px) → translateY(-4px)` yaylanması ile ritmik kobra salınımı (`rotate(-5deg) ↔ rotate(4deg)`).
    2. *Alevli Gözlerin Açılması (0.35s–0.95s):* Arbok'un göğsündeki sahte kobra gözleri akkor sarı/kırmızı ışıldar (`filter: drop-shadow(0 0 24px #facc15)`).
    3. *Tehdit Atılışı & Titreme (0.95s–1.35s):* Hedefe doğru `scale(1.08)` ile öne atılış ve `±0.5deg` mikro-titreme (`shudder tremor`).
    4. *Gölgeye Süzülerek Çekilme (1.35s–1.75s):* Aniden kaybolmak yerine kobra geriye doğru kıvrılarak süzülür (`translateY(22px)`, `scale(0.90)`, `opacity: 0`, `filter: blur(2px)`).
- **C. Pokémon Gücü Susturma Mührü (Silence Seal Rune) & Hipnotik Şok Dalgaları:**
  - Dark Arbok'un "Stare" saldırısının ana işlevi olan *"rakibin Pokémon Gücünü kapatma"* temasını yansıtmak adına; kobra göğsünden fırlayan iki kademeli dönen hipnoz dalgası (`gbaArbokHypnoRing1/2`), çift odaklı iyonize bakış huzmesi (`gbaArbokGazeBeams`) ve kurban kart üzerinde beliren kırmızı-sarı göz sembollü susturma mührü (`gbaArbokSilenceSeal`) entegre edildi.

### 10.10 Vaka Çalışması: Dark Arbok Zehir Buharı, Kostik Miazma & Tezgâh Süpürme (`poison_vapor`, 1820ms & `poison_vapor_bench`, 1320ms)

**Kaynak:** `BattleFXOverlay.tsx` L697 (dispatch `poison_vapor`), L1382–L1385 (`getFXDuration`: `poison_vapor` = 1820, `poison_vapor_bench` = 1320), L2682–L2890 · `index.css` L7680–L7850 · `preview_poison_vapor.html` · Görsel Varlıklar: `Arbok_PoisonFang_Maw.png` (Tier 1/2 Aktör `114px × 114px`, whiff: `78px × 78px`).

- **A. CAD Kesikli Çizgi, Düz Kayan Duman ve Yapay Ayrık Ögelerin Tasfiyesi (Anti-Pattern #24):**
  - **Sorun:** Eski implementasyonda zehir dumanı; kart üzerinde düz kayan sert vektörel bulut parçaları, kenarlara anlamsızca iliştirilmiş dik sinüzoidal çizgiler, ince tel kafes izlenimi veren yeşil eliptik bir çember (`border-2 border-lime-400`) ve tepeden düşen iki emoji/klipart damlasından ibaretti. Bu, hem Kural 7'yi ihlal ediyor hem de dövüşün ciddiyetini bozuyordu.
  - **Mimari Çözüm:** Tüm CAD çemberleri, ayrık çizgiler ve emoji damlaları tamamen söküldü. Yerine:
    1. Çok loblu, yumuşak Gaussian difüzyonlu (`blur(5px)–blur(9px)`) 3 katmanlı gerçekçi gaz dinamiği: Derin ametist taban sisi (`gbaVaporDeepPlume 1.82s`), kostik lime klor çekirdeği (`gbaVaporAcidCore 1.82s`) ve dönen türbülans girdapları (`gbaVaporSwirlingEddy 1.82s`).
    2. Sınırları belirleyen ince border yerine; merkezden yumuşak genleşen aerodinamik gaz basınç dalgası (`gbaVaporPressureWave 1.82s`, `radial-gradient` akkor halesi).
- **B. Birincil Görsel Aktör, Nozul Hizalanması ve Kart Sınırı İçi Kırpma (Nozzle Pinning & Anti-Bleed):**
  - **Sorun 1 (Kafa Kırpılması & Asimetrik Kayma):** Aktör tepeye çok yakın konumlandırıldığında kafa kubbesi kartın üst kenarından jilet gibi traşlanıyor; solda yeşil, sağda koyu mor kanatlar görsel sentroidi sola kaydırıyordu.
  - **Sorun 2 (Nozul Sapması & Kart Altına Taşma):** Arbok'un fildişi dişlerinden çıkması gereken buhar akıntısı gövdenin altından/boşluktan başlıyor ve gaz kütlesinin alt ucu hedef kartın alt sınırının dışına taşıyordu.
  - **Mimari Çözüm:**
    1. *Üst-Merkez Güvenli Kilitlenme:* Arbok üst-merkeze (`left: 50%`, `top: 22%`, `114px × 114px`) yerleştirildi; kafa kubbesinin daima kart içinde kalması için +11px güvenlik marjini korundu.
    2. *Püskürtücü Nozul Orijini (Maw/Nozzle Pinning):* Buhar fışkırma jeti doğrudan fildişi dişlerin tam arasına kenetlendi (`transform-origin: 50% 0%`, üst nozul kilitlenmesi). Eruption jetinin fışkırma koordinatları diş açıklığıyla pikseli pikseline örtüştürüldü.
    3. *Tavizsiz Kart İçi Hapsedilme (Anti-Bleed):* Aktif kart konteyneri `.card-fx-block-overlay rounded-xl overflow-hidden` vizörü ile mühürlendi; miazmanın alt kuyruğunun kart çerçevesinden dışarı taşması engellendi.
- **C. %10 Süre Genişletmesi & Akışkan İmpuls Kinematiği (1650ms $\rightarrow$ 1820ms):**
  - **Sorun:** Animasyon çok ani başlayıp aniden kesiliyordu; kinetik momentum hissi zayıftı.
  - **Mimari Çözüm:** Aktif animasyon süresi 1650ms'den **1820ms'ye (+%10.3)** genişletildi. Bu ek süreyle 4 fazlı apeks kinematiği zenginleştirildi:
    - *0.00s–0.24s (Süzülüş):* Yukarıdan kobra süzülmesiyle duruş alma (`scale 0.76 → 1.0`).
    - *0.24s–0.44s (Anticipation Zehir Birikimi):* Dişlerin arasında zehir bezi basınç birikimi akkor parlama flaşı (`gbaArbokMawGlow 0.24s–0.44s`) ve geriye yaylanma nefesi.
    - *0.44s–0.78s (Apeks Lunge & Akışkan Jet):* **ÖNE ŞİDDETLİ LUNGE!** (`scale 1.15`, mikro-titreme tansiyonu). Dişlerin tam arasından hedefin göğsüne doğru konik yüksek hızlı jet akımı patlar (`gbaVaporEruptionJet 1.82s`).
    - *0.78s–1.82s (Süblimleşme & Miazma Zarfı):* Arbok hafifçe geriye süzülerek (`filter: blur(4px)`, `opacity: 0`) sahneyi kurbanı boğan zehir dumanına bırakır.
- **D. 5 Katmanlı Tezgâh Süpürme Kimyasal Reaksiyonu (`poison_vapor_bench`, 1320ms):**
  - **Sorun:** Bench süpürme efekti çok kısa sürüyor (1200ms) ve aceleye getirilmiş basit yeşil lekelerden ibaret duruyordu.
  - **Mimari Çözüm:** Süre 1200ms'den **1320ms'ye (+%10.0)** çıkarıldı. Küçük tezgâh kartları (`size="sm"`, `110px × 151px`) üzerinde 5 katmanlı minyatür bir kimyasal reaksiyon orkestre edildi:
    1. *Katman 1:* Taban asit aurası (`gbaVaporBenchAura`),
    2. *Katman 2:* Çift loblu dönen miazma duman pufu (`gbaVaporBenchPuff`),
    3. *Katman 3:* 4 aşamalı kaynayan mikro asit kabarcıkları (`gbaVaporBenchBubble`),
    4. *Katman 4:* Havada süzülen mikro aerosol zerrecikleri (`gbaVaporBenchFlecks`),
    5. *Katman 5:* Darbe temas anında flaşlayan zehir hasarı doygunluğu (`gbaVaporBenchDamageFlash`).
  - `GameBoard.tsx` kademeli bench hasar gecikmeleri (+240ms, +350ms, +460ms) ile milisaniyesine senkronize edildi.
- **E. Önizleme Sahnesi & GameBoard Boyut Paritesi ve CSS Hijyeni (`preview_poison_vapor.html` Dersi):**
  - **Sorun 1 (Görsel ve Efektlerin Görünmeme Hatası):** Önizleme HTML dosyasında `.fx-layer` stil bloğunun kapanış parantezi (`}`) eksik olduğu için, altındaki 18 `@keyframes` kuralının tamamı `.fx-layer` içine gömülü (nested) kaldı. CSS spesifikasyonu gereği nested keyframe'ler geçersiz sayıldığından ve tüm elemanlar `opacity: 0` başladığından ekranda hiçbir efekt veya aktör görünmüyordu.
  - **Sorun 2 (Ölçek ve Durum Şeridi Eksikliği):** Önizleme kartları naif `184px × 256px` ve `110px × 152px` boyutlarındaydı; durum şeridi ve gerçek kart altlığı bulunmuyordu.
  - **Mimari Çözüm:**
    1. *CSS Parantez Dengesi & Bağımsız Keyframe Kuralı:* CSS blokları dengelendi (147 açılış = 147 kapanış) ve 18 keyframe'in tamamı doğrudan CSS root seviyesine taşındı (§6.5).
    2. *GameBoard Boyut Paritesi:* Aktif kart `184px × 253px` (`aspect-ratio: 600 / 825`), Yedek kartlar `110px × 151px` (`aspect-ratio: 600 / 825`) ölçülerine eşitlendi.
...
21. **Flexbox İçi `position: absolute` Statik Sıfır Tuzağı (Absolute-in-Flex Static Coordinate Drift Anti-Pattern):** Ebeveyn kapsayıcının `flex items-center justify-center` sınıflarına güvenilerek, mutlak pozisyonlu çocuk elemente `top: 50%; left: 50%` atanmaması; CSS keyframe'inde `transform: translate(-50%, -50%)` çalıştığında elemanın `(0, 0)` koordinatından sol-üste dışarı fırlaması hatası (Grimer Sticky Hands L16338 plazma küresi dersi). Çözüm: Keyframe'inde `translate(-50%, -50%)` kullanan her mutlak pozisyonlu elemana koşulsuz olarak `style={{ left: '50%', top: '50%' }}` koordinat kilitlenmesi verilmelidir (§6.4).
22. **Soyut Sahne ve Boyutsal Parite Kopukluğu Tuzağı (Abstract Stage & Dimension Desynchronization Anti-Pattern):** Önizleme (showcase) HTML sayfalarında rastgele sahne boyutları (`180px × 252px` vb.) ve boş siyah zeminler kullanılması; gerçek oyun içi aktif kart boyutları (`184px × 253px`, `600/825`) ve otantik kart altlıkları (`cards/*.jpg`) ile test edilmemesi sonucu kart içi anatomik hizalanma ve ölçek hatalarının fark edilememesi. Çözüm: Önizleme sahneleri oyun içi `CardView.tsx` aktif boyutlarına (184×253px) pikseli pikseline eşitlenmeli ve gerçek kart altlığı test modu (`cards/*.jpg`) barındırmalıdır (§6.5).
23. **Çift Görsel Kopya ve Opasite Kilitlenme Tuzağı (Duplicate Image Opacity Lock Trap):** Bir aktörün giriş ve çıkış animasyonunu yönetmek için aynı görseli iki ayrı DOM elemanına koyup, ilkinin keyframe'ine `animation-fill-mode: forwards` ile `opacity: 1` vermek; ikinci eleman erise dahi arkadaki birinci eleman ekranda kilitli kalacağından animasyonun son yarım saniyesinde donuk, cansız ve yapay bir fotoğraf tabakasının asılı kalmasına yol açar (Dark Arbok Stare dersi). Çözüm: Tek bir `<img>` kullanılmalı; giriş, nefes, darbe ve çıkış erimesi tek bir kesintisiz keyframe zinciriyle yönetilmelidir (§10.9.A).
24. **Spot Işığı Konisi, CAD Çember ve Kafa Kırpılma Tuzağı (Spotlight Cone, CAD Circle & Upper Head Clipping Anti-Pattern):**
    1. *Spot Işığı İllüzyonu:* Gaz veya buhar efektlerinde `clip-path: polygon(...)` ile düz kenarlı bir üçgen/trapezoid koni çizip havada statik tutmak; gaz akışkanlığı yerine sahneye tiyatro projektörü veya el feneri ışığı tutulmuş izlenimi verir. Çözüm: Düz konik poligon yasaktır; dişlerden fırlayan anlık (~300ms) yüksek hızlı akışkan buhar girdapları (`gbaVaporEruptionJet 1.82s`, impulse burst) kullanılmalı, ardından hızla miazma dumanına dönüşmelidir (§10.10.A).
    2. *Kafa Kırpılma Hatası (Top Edge Clipping):* Kartın `overflow: hidden` sınırlarını hesaba katmadan üst aktörü tepeye çok yakın (`top: 14%`, `-68% translateY`) yerleştirmek; aktörün kafa kubbesinin kartın üst kenarı tarafından jilet gibi kesilmesine yol açar. Çözüm: Aktör dikey güvenli bölgeye kilitlenmeli (`top: 22%`, `114px`), lunge ofsetleri hesaplanarak kafa kubbesinin daima kartın içinde (+11px marjin) kalması garanti edilmelidir (§10.10.B).
    3. *Jenerik Bench Çemberi (Tennis Ball Ring):* Bench süpürme hasarında ortada 1.5px yeşil border'lı içi dolu tenis topu gibi daireler patlatmak (Kural 7 ihlali). Çözüm: Border'lı geometrik çemberler tamamen yasaktır; yumuşak çift loblu Gaussian zehir pufu (`gbaVaporBenchPuff`) ve mikro akkor asit zerrecikleri (`gbaVaporBenchFlecks`) kullanılmalıdır (§10.10.D).
25. **Önizleme CSS Parantez Dengesizliği ve Gömülü Keyframe Tuzağı (Unclosed Brace & Nested Keyframe Anti-Pattern):**
    - Önizleme HTML dosyasındaki bir CSS seçici kuralında (örneğin `.fx-layer { ...`) kapanış süslü parantezinin (`}`) unutulması ve/veya dosya sonunda yetim bir parantez bırakılması.
    - *Sonuç:* Tarayıcının CSS ayrıştırıcısı (parser), bu seçiciden sonra gelen tüm `@keyframes` tanımlarını (örneğin 18 adet keyframe'in tamamını) ilgili sınıf seçicisinin içine gömülü (nested) kabul eder. Standart CSS spesifikasyonunda `@keyframes` kuralları stil seçicilerinin içine gömülemediğinden tarayıcı tüm keyframe'leri geçersiz sayarak çöpe atar. Tüm efekt katmanları ve aktörler başlangıçta `opacity: 0` ile kurgulandığından animasyon tetiklendiğinde ekranda hiçbir efekt veya görsel belirmez; geliştirici önizleme sayfasının tamamen "donduğunu" veya "boş kaldığını" zanneder.
    - *Çözüm:* Önizleme sayfalarındaki tüm `@keyframes` kuralları istisnasız CSS root seviyesinde tutulmalı; CSS süslü parantez bloklarının (`open === close`) dengesi her zaman otomatik testlerle veya linter'larla doğrulanmalıdır (§6.5, Kural 14).
26. **Nozul/Ağız Ekseni Kayması ve Kart Altına Taşma Tuzağı (Nozzle Offset Drift & Bottom Boundary Bleed Anti-Pattern):**
    - Bir Pokémon'un ağzından veya namlusundan fışkıran jet akımının (zehir buharı, su jeti vb.) aktörün anatomik ağız açıklığı yerine gövdenin altından ya da havada rastgele bir noktadan fışkırması; transform orijininin ağza kenetlenmemesi (`transform-origin: 50% 0%` eksikliği) ve konteyner dikey pozisyonunun (`top: 22%`) yanlış hesaplanması sonucu gaz kütlesinin hedef kartın alt sınırından (`overflow-hidden` vizörü dışına) kontrolsüzce taşması.
    - *Çözüm:* Fışkırma jeti aktörün ağız koordinatlarına pikseli pikseline kenetlenmeli (`transform-origin: 50% 0%`), lunge esnasında da ağız açıklığı ile senkronize ilerlemeli; akıntı konteyneri hedef kartın fiziksel vizörü içinde (`overflow-hidden rounded-xl`) kilitlenmelidir (§6.2, Kural 8).
27. **Gövdesiz Organ / Yüzen Parça Tuzağı (Severed Organ / Floating Limb Anti-Pattern):**
    - Bir Pokémon'un güçlü imza saldırısını betimlerken gövdeden koparılmış parçaların (sadece kopuk bir kabuk, havada asılı duran yüzen tüfekler veya havada yüzen kopuk kollar) kullanılması; aktörün anatomik ve fiziksel ağırlığını yok ederek oyuncuda çocuksu/yarım kalmış bir maket hissi uyandırır.
    - *Çözüm:* Tam vücut Ken Sugimori suluboya aktörü (`blastoise_full_body.png`), yere sağlam basan yerçekimli sumo girişi (`gbaBlastoiseFullBodyMotion 1.9s`), anatomik nozul merkezlerine pikseli pikseline kenetlenen ikili namlu ağzı parlamaları ve hidrolik basınç geri tepmesi ile mikro-titreme sarsıntısı kullanılmalıdır (§10.11, Kural 1, 2, 8).
28. **Sert Çizgi / Lazer Çubuğu Su Akıntısı Tuzağı (Rigid Line Stick / Laser Beam Water Anti-Pattern):**
    - Su jetlerini veya hidro akıntıları betimlerken tek bir `<line>` veya tekdüze 20–26px genişliğinde düz SVG yolu kullanmak; suyun akışkan hidrodinamik genleşmesini (dar nozul çıkışı $\rightarrow$ genişleyen dalga tepesi), iç kavitasyon omurgasını, helikal girdaplarını ve serbest sıçrayan damlacıklarını yok sayarak suyu bir Star Wars lazer çubuğuna veya floresan lambaya çevirir.
    - *Çözüm:* İkili/üçlü kademeli genişleyen Bézier hacmi (namluda 14px, tepede 36px), dış serulean kabuk (`#0284c7`), basınçlı ara akıntı (`#38bdf8`) ve akkor beyaz kavitasyon çekirdeği (`#ffffff`); hedef temas noktasında çok loblu Tsunami Taç Sıçraması (`gbaHydroSplashCrown`) ve kartı yıkayan şelale akıntısı (`gbaHydroDelugeCascade`) kullanılmalıdır (§10.11, Kural 1, 7).
29. **Asimetrik Pozda Simetrik Koordinat Varsayımı Tuzağı (Symmetric Coordinate Assumption in Asymmetric Stance):**
    - 3/4 açıyla duran veya dinamik perspektifli bir Pokémon'un sağ ve sol organlarına (namlular, gözler, kollar) körü körüne simetrik CSS ofsetleri (`calc(50% ∓ Xpx)`) atamak; bir tarafı doğru hizalarken diğer tarafı vücudun alakasız bir yerine (örneğin Blastoise'un sağ namlu alevini boynuna/kulağına) bindirir.
    - *Çözüm:* Her iki nozul/organ koordinatı görselin gerçek piksel matrisinden bağımsız olarak okunmalı (`X_left = 23.65%`, `X_right = 94.62%`); kart genişliğine göre bağımsız asimetrik formüllerle (`calc(50% - 30px)` ve `calc(50% + 51px)`) kenetlenmelidir (§10.11, Kural 8).
30. **Çapraz Çatı Makası Su Jeti Tuzağı (Crossed Tent Stream Anti-Pattern):**
    - Su jetlerini iki ayrı namludan fırlatırken, namluların dışa bakan fiziksel eksenlerini (-28° ve +40°) yok sayarak akıntıları hedefe doğru doğrudan doğrusal olarak içe bükmek (-27° ve +16° gibi); su kütlelerinin hedefin önünde bir çatı makası ("A" harfi) gibi çapraz kesişmesine ve namlu delikleriyle bağlantısız görünmesine yol açar.
    - *Çözüm:* Kavisli Kıskaç (Curved Pincer) trajektorisi kullanılmalıdır: Her akıntı önce namlunun gerçek mekanik açısıyla fırlar (Sol: -28°, Sağ: +40°), ardından doğal bir hidrodinamik Bézier kavis (`M... C...`) çizerek hedefin merkezine odaklanır. Akıntılar birbirini asla kesmez; hedefi iki yandan çevreleyerek vurur (§10.11, Kural 7, 8).
31. **Tarak Dişi Şelale ve Düz Çizgi Tufan Tuzağı (Comb Teeth Water Cascade Anti-Pattern):**
    - Su çarpması sonrasında kartı yıkayan su kaskadını (deluge) veya damlacıkları, yukarıdan aşağıya inen 4–5 adet düz düşey çizgi veya çalakalem çizilmiş çubuklarla betimlemek; oyuncuya su akıntısı yerine "hapishane parmaklıkları" veya "tarak dişleri" hissi verir.
    - *Çözüm:* Undulating (dalgalanan) şeffaf su perdesi, 5 adet kıvrımlı ve kalınlığı değişken Bézier su deresi (`M... C... Q...`), kart tabanında toplanan köpüklü su halkaları ve yerçekimiyle dökülen parlak su damlası küreleri kullanılmalıdır (§10.11, Kural 1, 7).
32. **Ters 'J' Şeklinde Bükülen Su Kancası ve Statik SVG Konteyner Tuzağı (Reverse 'J' Water Hook & Static Box Container Trap):**
    - Su jetlerinin hedefin önüne ulaşabilmesi için SVG yollarının geriye kıvrılan baston/şemsiye sapı veya ters 'J' kancası şeklinde bükülmesi; ayrıca su akıntılarının dar bir SVG konteyneri (`scale` veya `clipPath`) içinde ölçeklenerek belirmesi sonucu kendilerine ait bir arka planı varmış gibi (bounding-box artifact) hissettirmesi.
    - *Çözüm:* Saldırgan Pokémon'un vücut pozisyonu ve namlu yönelimi doğrudan hedefe bakacak şekilde seçilmeli (hücum duruşlu `Blastoise_raw_edited_002.png`, Sol: `calc(50% - 16px)` @ 115px [+10°], Sağ: `calc(50% + 54px)` @ 98px [-27°]); su akıntısı hedefe doğru doğrusal konik genişlemeyle fırlatılmalı (`scale(0.15, 0.05)` $\rightarrow$ `scale(1.05, 1.02)`), statik bounding box yerine nozuldan fışkıran 3 katmanlı akışkan Bézier dalga kretleri, kavitasyon omurgası ve sarmal girdap şeritleri kullanılmalıdır (§10.11, Kural 1, 7, 8).
33. **Blast Kütlesi Vizör Kırpılması, Merkezleme Boşluğu Yokluğu ve Yapay Tekdüze Damla Tuzağı (Blast Visor Clipping, Actor Center Cramping & Artificial Droplet Trap):**
    - Ana blast darbe kütlesini kartın sol üst kenarına çok yakın konumlandırarak (`top: 26px`, `left: 18%`) dairesel patlamanın 3/5'lik kısmının kart sınırları (`overflow-hidden`) dışında kalmasına ve yalnızca sağ alttan 2/5'lik bir parçanın görünmesine neden olmak; ayrıca Blastoise gibi geniş hücum duruşlu bir aktörü kartın tam merkezine oturtup sağ namluya kart içinde hareket mesafesi bırakmamak ve namluları hedefe odaklamak için sol namluyu mantıksız şekilde sol tarafa bükmek (-28°); zemin mavi parlamasını aktörün vücut ekseninden koparıp kartın sağ alt köşesine savurmak; ve patlama sonrası su saçılmasını sadece 3 adet büyük stilize damlayla geçiştirmek.
    - *Çözüm:* Ana dairesel blast kütlesi kartın vizörü içinde tamamen görünür olacak şekilde merkeze ve hedef bölgesine (`top: 54px`, `left: calc(50% - 6px * wi)`) çekilmeli; Blastoise aktörü merkezden sola (`left: calc(50% - 16px * wi)`) ötelenerek sağ namluya ferah bir fışkırma mesafesi tanınmalı, sol namlu (-18°) ve sağ namlu (+30°) hedefe doğal eksenlerle yönlenmeli; zemin mavi glow'u Blastoise'un gövde merkezine ve iki ayağının tabanına kilitlenmeli; su damlacıkları ise 10 farklı doğal açıda balistik yaylar çizen, farklı boyutlarda (`6px`–`12px`) ve akışkan mikro kavitasyon zerreleri içeren radyal bir dispersiyonla zenginleştirilmelidir (§10.11, Kural 1, 3, 7, 8).
34. **Yapay Sivri Uçlu Taç Sıçraması, Üçgen Parıltı Dişleri ve Kopan Akıntı Tuzağı (Spiky Crown Splash, Triangular Glow Teeth & Detached Stream Trap):**
    - *Yapay Sivri Taç:* Blast darbe kütlesi etrafındaki su sıçramasını 6 adet sert, iğne uçlu üçgen dişle çizip uçlarına dairesel beyaz noktalar koymak; akışkan hidrodinamik bir su patlaması yerine çizgi film/klipart bir prenses tacı izlenimi verir.
    - *Üçgen Parıltı Dişleri:* İç beyaz köpük katmanında keskin geometrik açılar (`(63, 30)` ve `(97, 30)`) bırakmak; dış mavi katman sönerken son fazda merkezde iki adet sert beyaz köpekbalığı dişi/üçgeninin yapay olarak asılı kalmasına yol açar.
    - *Kopan Akıntı Hatası:* Su akıntıları son fazda (%76–%100) `translateY(-24px)` veya `translateY(-28px)` ile namludan kopup yukarı fırlatıldığında, genişleyen akıntı kafaları hedefin merkezine hücum ederek iki ayrı beyaz üçgen gölge gibi birikir.
    - *Çözüm:* 10 loblu organik Ken Sugimori suluboya kavisli dalga mantosu (`M80 106 C64 106...`), dairesel difüzyonlu aeratör köpük tabakası (`tsunamiFoamWash_bfx`) ve kıvrımlı dalga kret çizgileri kullanılmalı; su akıntıları keyframe'lerinde kopma kaldırılıp nozul tabanında kilitli kalarak yerinde Gaussian buğulanmayla yumuşakça sönümlenmelidir (§10.11, Kural 1, 2, 7).
...
15. **Flexbox İçi Eş Eksenli Merkezleme ve Önizleme Parite Kontrolü (Flexbox Coaxial Coordinate & Preview Parity Check):** Keyframe'inde `translate(-50%, -50%)` kullanan her mutlak pozisyonlu elemanın açıkça `top: 50%; left: 50%` koordinatlarına kilitlendiği doğrulandı mı? Önizleme testleri soyut kutular yerine tam kart boyutunda (`184px × 253px`) ve gerçek kart illüstrasyonları üzerinde yapıldı mı (§6.4, §6.5, Anti-patern #21, #22)?
16. **Tekil Aktör DOM Bütünlüğü ve Opasite Kilitlenme Kontrolü (Single Actor DOM Integrity & Opacity Lock Check):** Aktör üzerinde giriş (`materialize`) ve çıkış (`dissolve`) için mükerrer `<img>` kopyaları açılmadığı; `forwards` ile `opacity: 1` seviyesinde havada asılı kalan katmanların bulunmadığı doğrulandı mı (§10.9.A, Anti-patern #23)?
17. **Zehir Miazması, Dikey Güvenli Bölge ve Tezgâh Flecks Kontrolü (Toxic Miasma, Vertical Safe-Zone & Bench Flecks Check):** Püskürme konisinin statik spot ışığı (`clipPath polygon`) gibi kalmadığı, anlık akışkan impuls jeti olarak patlayıp dumanlaştığı; kobra başının kartın üst kenarından traşlanmadığı (`top: 22%`, +11px güvenli marjin); tezgâh süpürme efektinde border'lı yeşil tenis topu dairelerinin bulunmadığı, organik çift loblu puf ve mikro akkor zerreciklerin (`gbaVaporBenchFlecks`) oynatıldığı doğrulandı mı (§10.10, Anti-patern #24)?
18. **Önizleme CSS Parantez Dengesi ve Keyframe Root Seviyesi Kontrolü (Preview CSS Brace Balance & Root Keyframe Check):** Önizleme HTML dosyalarında açılan ve kapanan süslü parantezlerin (`{` ve `}`) eşit sayıda olduğu; hiçbir `@keyframes` kuralının bir sınıf seçicisi içine gömülmediği ve tüm animasyonların bağımsız çalışabildiği doğrulandı mı (§6.5, Anti-patern #25)?
19. **Nozul Püskürtme Noktası, Ağız Hizalaması ve Bench Kart Paritesi Kontrolü (Nozzle Origin, Mouth Alignment & Bench Parity Check):** Ağızdan veya organdan püsküren akıntıların başlangıç orijininin aktörün anatomik ağız açıklığına kilitlendiği (`transform-origin: 50% 0%`, `top: 22%`), kartın alt sınırından taşma yapmadığı; bench kartlarının masaüstü `GameBoard.tsx` ve `CardView.tsx` boyutları olan `110px × 151px` ölçüsü ve durum şeridiyle test edildiği doğrulandı mı (§6.2, §6.3, §6.5, Anti-patern #26)?
20. **İkili Nozul Simetrisi, Hidrolik Geri Tepme ve Whiff İskalama Kontrolü (Dual Nozzle Symmetry, Hydraulic Recoil & Whiff Check):** İkili namlulardan fışkıran jetlerin simetrik nozul eksenlerine (`calc(50% ∓ 28px)`) kenetlendiği, jetlerin ±11° ile hedef kart tepe merkezine odaklandığı; atış anında sumo geri tepmesi ve mikro-titremenin çalıştığı; whiff durumunda su püskürtülmeyip aktörün küçülerek cılızca çekildiği doğrulandı mı (§10.11, Anti-patern #27)?
21. **Volumetrik Akışkan Torrenti, Asimetrik Nozul ve Taç/Tufan Kontrolü (Volumetric Fluid Torrent, Asymmetrical Nozzles & Crown Deluge Check):** Su akıntılarının düz lazer çizgisi yerine genişleyen Bézier hacmine ve kavitasyon omurgasına sahip olduğu; 3/4 pozlardaki nozulların bağımsız koordinatlarla (`-30px` ve `+51px`) tam namlu deliklerine oturduğu; namlu alevlerinin namlu açılarıyla (-32° / +45°) yönlendiği; hedef temasında Tsunami taç sıçraması ve kartı yıkayan tufan akıntısının çalıştığı doğrulandı mı (§10.11, Anti-patern #28, #29)?
22. **Kavisli Kıskaç Trajektorisi, Akkor Basınç Kubbesi ve Akışkan Şelale Kontrolü (Curved Pincer Trajectory, Concussion Dome & Fluid Deluge Check):** Su akıntılarının namlu deliklerinin fiziksel yönlerinde (-28° ve +40°) fırlayıp kavis çizdiği, hedefin önünde çapraz kesişmediği ("A" harfi oluşturmadığı); çarpma anında 4 uçlu elmas yıldız veya ham çemberler yerine akkor basınç kubbesi (`gbaHydroConcussionDome`) ve 14 loblu Tsunami Taç Sıçramasının patladığı; su kaskadının düz parmaklıklar yerine kıvrımlı Bézier dereleriyle kartı yıkadığı doğrulandı mı (§10.11, Anti-patern #30, #31)?
23. **Doğrudan Atış Vektörleri, Hücum Duruşlu Nozul Ekseni ve Akışkan Dalga Kretleri Kontrolü (Direct Fire Vectors, Forward-Aim Nozzle Axes & Fluid Wave Crests Check):** Su jetlerinin ters 'J' veya baston kancası yapmadığı, hücum duruşlu namluların gerçek eksenlerinden (+10° ve -27°) hedefin merkezine doğru konik olarak fışkırdığı; su kütlelerinin statik bir SVG kutusu hissi vermeden nozuldan kopup genişleyen 3 katmanlı hacimsel akışkan Bézier dalgaları oluşturduğu; hedef temasında akkor hidro-basınç kubbesi, 14 loblu Tsunami taç sıçraması ve kartı yıkayan şelale kaskadının çalıştığı doğrulandı mı (§10.11, Anti-patern #32)?
...
| 10 | 2026-09-25 | Grimer Sticky Hands ve Muk Sludge Deluge eş eksenli hizalanma ve ölçeklendirme düzeltmesi: Flexbox içi `position: absolute` statik sıfır koordinat tuzağı giderildi (`left: 50%; top: 50%` kilitlenmesi, §6.4); `preview_grimer_muk.html` oyun içi `CardView.tsx` aktif kart boyutlarıyla (`184px × 253px`, `600/825`) ve otantik kart altlıklarıyla (Magnemite #53, Grimer #48) tam pariteye kavuşturuldu (§6.5). Kademeli ve dengeli ~%12–%14 ölçek kalibrasyonu uygulandı (Grimer Sticky Hands 140px→122px, Muk Deluge 144px→126px, Nasty Goo 160px→140px, Minimize 140px→122px, plazma küresi ve keyframe tepe çarpanları temperlendi; §10.6.C, §10.6.E). Anti-patern #21 ve #22 ile Ayar Turu #15 maddesi eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_grimer_muk_animations.py` 40/40 PASS |
| 11 | 2026-09-25 | §10.9 Dark Arbok Stare (Cobra Stare) 5-Katmanlı Mimari ve Kesintisiz Momentum Modernizasyonu: Son 0.45s'lik statik donma (Anti-Idle %15 kuralı ihlali) ve çift `<img>` opaklık kilitlenme hatası (L14873 vs L14895) giderildi; `getFXDuration('cobra_stare') = 1750ms` eklendi. Tekil aktörde 4-fazlı kobra fiziği (`gbaArbokCobraMotion 1.75s`: yükselme, ritmik salınım -5°↔+4°, öne lunge mikro-titreme, sinuous retreat), alevli göğüs sahte gözleri ışıması (`gbaArbokChestPulse`), eşmerkezli hipnotik şok dalgaları (`gbaArbokHypnoRing1/2`), odaklı bakış huzmeleri (`gbaArbokGazeBeams`), Pokémon Gücü susturma mührü (`gbaArbokSilenceSeal`) ve 6 yönlü zerre saçılımı (`gbaArbokGazeMote`) entegre edildi. Anti-patern #23 ve Ayar Turu #16 eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_dark_arbok_stare.py` 35/35 PASS · `python scratch/test_grimer_muk_animations.py` 40/40 PASS |
| 12 | 2026-09-25 | §10.10 Dark Arbok Poison Vapor (Zehir Buharı) Akışkan Aerodinamik Miazma & Üst-Merkez Kobra Lunge Modernizasyonu: İlkel CAD kesikli çizgi (`strokeDasharray="5 4"`), kenarlara yapışık dik sinüzoidal çizgiler, ince tel çember (`border-2 border-lime-400`) ve tepeden düşen emoji/klipart damla hataları (Anti-patern #24) tamamen tasfiye edildi. Birincil aktör olarak sol üst köşeden üst-merkeze (`left: 50%`, `top: 14%`, `114px × 114px`, whiff: `78px`) taşınan Ken Sugimori suluboyası `Arbok_PoisonFang_Maw.png` ile 4-fazlı apeks kobra kinematiği (`gbaArbokVaporLunge 1.65s`), fildişi dişlerden hedefe doğru konik yüksek hızlı jet akımı (`gbaVaporEruptionJet 1.65s`), sıfır CAD sınırına sahip aerodinamik gaz basınç dalgası (`gbaVaporPressureWave 1.65s`), 3 katmanlı yumuşak Gaussian difüzyonlu gaz lobları (derin ametist `gbaVaporDeepPlume`, kostik lime `gbaVaporAcidCore`, türbülanslı girdap `gbaVaporSwirlingEddy`), 6 balistik mikro asit zerresi (`gbaVaporSpurtParticle`), 6 fokurdayan parlak kabarcık (`gbaVaporBoilingBubble`), zehir durumu doygunluğu (`gbaVaporPoisonSaturation`) ve tezgâh süpürme mini miazması (`gbaVaporBenchPuff/Aura/Sparks 1200ms`) entegre edildi. `preview_poison_vapor.html` baştan aşağı yenilendi. Ayar Turu #17 eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_dark_arbok_poison_vapor.py` 49/49 PASS · `python scratch/test_dark_arbok_stare.py` 35/35 PASS |
| 13 | 2026-09-25 | §10.10.B–E Dark Arbok Poison Vapor Ağız Hizalanması, Akışkan İmpuls/Anticipation ve Tezgâh Viskozite Overhaul'ı: Kullanıcı geri bildirimleri uyarınca 3 kritik iyileştirme tamamlandı: 1) Sol tarafa kayma hissiyatını yaratan asimetrik renk dağılımı (solda yeşil, sağda koyu mor kanatlar) ve kütle yığılması giderildi; dişlerin tam merkez aksına (`left: 50%`, `top: 25%`) kilitlenmiş akkor beyaz-yeşil jet çekirdeği ve simetrik çift kanatlı buhar fısıltıları entegre edildi, gaz kütlesi kartın merkez aksında dengelendi. 2) Ani gaz püskürmesi yumuşatılarak fildişi dişler arasında zehir bezi basınç birikimi (`gbaArbokMawGlow 0.24s–0.44s`) ve kademeli slow-in akışkan ivmelenme eğrisi (`gbaVaporEruptionJet`) ile kinematik akıcılık sağlandı; içi boş CAD halkası hissi veren basınç dalgası dolgulu yumuşak Gaussian şok dalgasına dönüştürüldü. 3) Tezgâh hasarı (bench miasma) basit renk lekelerinden kurtarılarak zemin aurası, ters dönen 3 loblu miazma (`gbaVaporBenchPuff`), 4 kademeli kaynayan asit köpüğü (`gbaVaporBenchBubble`) ve mikro aerosol parçacıklarıyla 4 katmanlı viskoz bir kimyasal reaksiyona yükseltildi. `BattleFXOverlay.tsx`, `index.css` ve `preview_poison_vapor.html` tam senkronize edildi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_dark_arbok_poison_vapor.py` 53/53 PASS · `python scratch/test_dark_arbok_stare.py` 35/35 PASS |
| 15 | 2026-09-25 | §10.9.B Dark Arbok Stare (Cobra Stare) Hipnotik Şok Dalgaları (Layer 3) & CAD Tasfiyesi Overhaul'ı: Kullanıcı geri bildirimi ve görsel teyit doğrultusunda, son fazda ekranda dönen çiğ sarı ve mor çizgili elipsler (ham CSS `border: 2px solid yellow`, `border: 1px solid purple` tel kafes ovals; Kural 7 ihlali) tamamen tasfiye edildi. Yerine 5-katmanlı mimariye ve Sugimori suluboyasına sadık 2 kademeli organik Bézier SVG psiyonik şok dalgası entegre edildi: Halka 1 (Güneş/Kehribar Hipnoz Dalgası, 136×116px): Akkor beyaz çekirdek $\rightarrow$ limon sarısı $\rightarrow$ neon altın $\rightarrow$ sıcak kehribar gradyanı ve hacimsel psiyonik eter örtüsü (`pvgGoldVeil`). Halka 2 (Karanlık Boşluk Çarpılma Dalgası, 176×152px): Akkor lavanta $\rightarrow$ neon leylak $\rightarrow$ psiyonik mor $\rightarrow$ derin uçurum eflatunu gradyanı ve boşluk örtüsü (`pvgVoidVeil`). Keyframe'lerde akışkan genişleme ve yumuşak Gaussian difüzyon sönümlenmesi (`filter: blur(1.5px)` $\rightarrow$ `blur(4px)` $\rightarrow$ `blur(8px)` ile %84–%100 aralığında havada süzülerek dağılma) sağlandı; son fazda sert eliptik çizgilerin asılı kalması engellendi. `preview_dark_arbok.html`: 184px × 253px GameBoard paritesi, durum şeridi (HP + segmentli yeşil pips), otantik kart modu toggle'ı (Base Set Blastoise #2) eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_dark_arbok_stare.py` 35/35 PASS · `python scratch/test_dark_arbok_poison_vapor.py` 58/58 PASS |
| 16 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Tam Vücut Ken Sugimori Stok Aktör & İkili Namlu Nozul Kenetlenmesi Modernizasyonu: Kopuk sırt kabuğu ve mekanik tüfekler (`blastoise-hydro-cannon.png`, Anti-patern #27) yerine tam vücut 1996 Ken Sugimori suluboyası (`blastoise_full_body.png`, 1024×988, Tier 1 Apex 114px) entegre edildi. İkili namlu merkezlerine (`calc(50% - 28px)` ve `calc(50% + 29px)`) kenetlenen nozul flaşları (`gbaHydroCannonMuzzle`) ve ±11° konverjans açılı çift hidro su sütunları (`gbaHydroCannonJetLeft/Right`) yerleştirildi. 4-fazlı sumo duruşu kinematiği (`gbaBlastoiseFullBodyMotion 1.9s`: yerçekimli sumo girişi, atış geri tepmesi, sürekli hidrolik mikro-titreme ve feathered dissolve) ile Anti-Idle %15 kuralı sağlandı. Whiff durumunda cılız geri çekilme (`gbaBlastoiseWhiffRecede`) ve jet/flaş bastırması uygulandı. `getFXDuration('hydro_pump_cannons')` 1900ms ile CSS keyframe'lerine milisaniyesine senkronize edildi. `preview_blastoise.html` (184×253px GameBoard paritesi, Base Set Blastoise #2 altlığı, durum şeridi, hız kontrolü) oluşturuldu. Anti-patern #27 ve Ayar Turu #20 eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_full_body.py` 34/34 PASS · `python scratch/test_dark_arbok_stare.py` 35/35 PASS |
| 17 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Volumetrik Hidrodinamik & Asimetrik Nozul Overhaul'ı: Kullanıcı geri bildirimi ve görsel teyit uyarınca `Blastoise_raw_edited_001.png` onaylı varlığı Lanczos optimizasyonuyla sisteme alındı (`1023×974`). Lazer çubuğunu andıran naif düz çizgiler (Anti-patern #28) ve asimetrik 3/4 duruşta simetrik koordinat varsayımı (Anti-patern #29) tamamen tasfiye edildi. Bağımsız nozul koordinatları (Sol: `calc(50% - 30px)` @ 102px, -32° namlu açısı, +16° içe odaklanma; Sağ: `calc(50% + 51px)` @ 100px, +45° namlu açısı, -27° içe odaklanma) piksel hassasiyetiyle kilitlendi. Namluda 14px'ten başlayıp hedefte 36px'e genişleyen 3 kademeli akışkan Bézier su sütunları (serulean kabuk, basınçlı ara akıntı, akkor beyaz kavitasyon omurgası), açılı namlu alevleri, hedefte çok loblu Tsunami Taç Sıçraması (`gbaHydroSplashCrown`), kartı boydan boya yıkayan Şelale Tufanı (`gbaHydroDelugeCascade`), 4 yönlü balistik kavitasyon damlaları ve savunan kart hidrolik mikro-titremesi (`gbaDefendingCardHydraulicTremor`) entegre edildi. `preview_blastoise.html` tam pariteyle yenilendi. Anti-patern #28, #29 ve Ayar Turu #21 eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_full_body.py` 44/44 PASS |
| 18 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Namlu Eksenli Kavisli Kıskaç Trajektorisi, Organik Hidrodinamik Hacim, Akkor Basınç Kubbesi & Akışkan Şelale Overhaul'ı: Kullanıcı geri bildirimi ve görsel teyit uyarınca 4 temel kusur giderildi: 1) Su jetlerinin hedefin önünde "A" çadırı gibi çapraz kesişmesi (Anti-patern #30) giderildi; sol namlu (-28°) ve sağ namlu (+40°) eksenlerinden fırlayıp hedefe doğru kavis çizen Kavisli Kıskaç (Curved Pincer) Bézier rotası (`M... C...`) entegre edildi. 2) Düz poligonlar yerine 3 katmanlı hidrodinamik su akıntısı (akkor beyaz kavitasyon omurgası, basınçlı camgöbeği ara akıntı, serulean dış mantolama, sarmal girdap şeritleri ve merkezkaç su damlacıkları) uygulandı. 3) Yavan 4 uçlu elmas yıldız patlaması ve CAD halkaları çöpe atılarak akkor hidro-basınç kubbesi (`gbaHydroConcussionDome`), 2 kademeli şok halkaları ve 14 loblu Tsunami Taç Sıçraması (`gbaHydroTsunamiCrown`) inşa edildi. 4) Tarak dişi hissi veren düz düşey çizgiler (Anti-patern #31) kaldırılarak dalgalanan şeffaf su perdesi, 5 organik kıvrımlı su deresi ve taban sıçrama köpüğü (`gbaHydroDelugeCascade`) entegre edildi. `BattleFXOverlay.tsx`, `index.css` ve `preview_blastoise.html` tam pariteyle güncellendi. Anti-patern #30, #31 ve Ayar Turu #22 eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_full_body.py` 44/44 PASS · `python scratch/test_dark_arbok_stare.py` 35/35 PASS · `python scratch/test_dark_arbok_poison_vapor.py` 58/58 PASS |
| 19 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Hücum Duruşu & Doğrudan Atış Vektörleri Overhaul'ı: Kullanıcı geri bildirimleri ve eklenen `Blastoise_raw_edited_002.png` onaylı varlığı uyarınca yapay ters 'J' / baston kancası trajektorisi ve statik SVG kutu görünümü (Anti-patern #32) tamamen tasfiye edildi. Namluları doğrudan rakibe yönelmiş hücum duruşlu Ken Sugimori stok aktörü (`blastoise_hydro_pump_actor.png`, 1381×1346, Tier 1 Apex 118px) sisteme alındı. Namlu merkezlerine (Sol: `calc(50% - 16px)` @ 115px; Sağ: `calc(50% + 54px)` @ 98px) kenetlenen doğrudan atış vektörleri (Sol: +10°, Sağ: -27°) inşa edildi. Nozuldan hedefe fırlayan 3 katmanlı volumetrik Sugimori dalga kretleri, kavitasyon omurgası ve sarmal girdap şeritleri içeren akışkan su torrenteri (`gbaHydroTorrentL/R`), nozul ağzı su halkaları (`gbaHydroMuzzleCollarL/R`), hedef temasında concussive hydro dome (`gbaHydroConcussionDome`), akışkan şok halkası (`gbaHydroShockRing`), 14 loblu Tsunami Taç Sıçraması (`gbaHydroTsunamiCrown`) ve şelale kaskadı (`gbaHydroDelugeCascade`) entegre edildi. `BattleFXOverlay.tsx`, `index.css` ve `preview_blastoise.html` tam pariteyle güncellendi. Anti-patern #32, Ayar Turu #23 ve Revizyon #19 eklendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_hydro_pump.py` 48/48 PASS |
| 20 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Doğal Namlu Eksenleri Kalibrasyonu (-28° Sol, +36° Sağ) & Enerji Küresi/Parmaklık Deluge Tasfiyesi: Kullanıcı geri bildirimi ve görsel teyit uyarınca sağ namlunun ters açıyla sol yukarıya bükülmesi hatası giderildi; sol namlu doğrudan sol-yukarıya (-28°), sağ namlu doğrudan sağ-yukarıya (+36°) fışkıracak şekilde mekanik namlu eksenlerine tam kilitlendi. Havada asılı duran yapay beyaz/mavi enerji küreleri (`gbaHydroMuzzleCollarL/R`) ve hapishane parmaklığı izlenimi veren dikey çizgili su perdesi (`gbaHydroDelugeCascade`) tamamen tasfiye edildi. Kullanıcının beğendiği sol-üst ana kütle yuvarlak blast efekti (`gbaHydroConcussionDome`) ve sıçrayan su damlacıkları korundu; sağ jet için de kavitasyon patlama kütlesi eklenerek simetrik Sugimori tsunamisi tamamlandı. Zemin aurası Blastoise'un ayaklarının tabanına yumuşak köpük difüzyonu olarak oturtuldu. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_hydro_pump.py` 48/48 PASS |
| 21 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Blast Kütlesi Merkezleme & Vizör İçi Tam Görünürlük, Aktör Sola Öteleme Açıklığı (-16px), Zemin Glow Ayak Kenetlenmesi ve 10-Vektörlü Doğal Radyal Kavitasyon Saçılımı: Kullanıcı geri bildirimi ve görsel teyit uyarınca 4 temel kusur giderildi: 1) Sol-üst kenarda 3/5'i kırpılan blast kütlesi daha merkeze (`top: 54px`, `left: calc(50% - 6px * wi)`) taşınarak 92×92px dairesel kütlenin %100'ünün kart sınırları içinde görünmesi sağlandı. 2) Blastoise aktörü merkezden 16px sola (`left: calc(50% - 16px * wi)`) kaydırılarak sağ namluya kart içinde ferah hareket/mesafe alanı açıldı; sol namlu (-18°) ve sağ namlu (+30°) hedefe mükemmel doğal doğrultu kazandı. 3) Sağ alt köşeye kayan zemin mavi glow efekti Blastoise'un yeni gövde merkezine ve iki ayağının tabanına (`left: calc(50% - 16px * wi)`) kilitlendi. 4) Stilize 3 büyük damla yerine; 10 adet balistik kavitasyon damlası ve su zerresi (kuzey-batı, zirve dik plume, kuzey-doğu, yanal püskürmeler ve seken mikro damlacıklar) ile zengin, doğal bir radyal saçılım inşa edildi. `BattleFXOverlay.tsx`, `src/index.css` ve `preview_blastoise.html` tam pariteyle güncellendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_hydro_pump.py` 48/48 PASS |
| 22 | 2026-09-25 | §10.11 Blastoise Hidro Pompa (Hydro Pump) Kuyruk Sınır Teğeti (-34px Sola Öteleme), Çift Namlu Konverjans Açısı (+28° Sol, +15° Sağ), Zemin Glow Ayak Tabanı Kenetlenmesi (-38px) & Blast Kütlesi Tam Merkezleme (-2px, top 52px): Kullanıcı geri bildirimi ve görsel/geometrik denetim doğrultusunda 4 kusur kalibre edildi: 1) Blastoise aktörü kuyruk ucu rakip kartın sol sınırına tam teğet (`left: 0px`) olacak şekilde `left: calc(50% - 34px * wi)` konumuna çekildi. 2) Sağ namlunun boşa gitmesi ve sol namlunun dışa taşması giderildi; sol namlu (`calc(50% - 54px)`, `bottom: 105px`) +28° açıyla, sağ namlu (`calc(50% - 28px)`, `bottom: 101px`) +15° açıyla orantılı olarak sağa/merkeze doğru yatırılarak her iki jetin de kartın üst merkezindeki hedef blast kubbesinde kusursuz konverjansla buluşması sağlandı. 3) `preview_blastoise.html` içindeki keyframe'lerde eksik olan `transform: translateX(-50%)` eklenerek zemin mavi aurasının sağ alt köşeye kaçması önlendi; 112px genişliğindeki difüzyon aurası Blastoise'un iki ayağının tabanının tam ortasına (`left: calc(50% - 38px * wi)`) kilitlendi. 4) Concussive hydro dome, şok halkaları ve Tsunami taç sıçraması su jetlerinin nihai varış noktasına göre kartın tam merkez vizörüne (`top: 52px`, `left: calc(50% - 2px * wi)`) oturtuldu. `BattleFXOverlay.tsx`, `src/index.css` ve `preview_blastoise.html` tam pariteyle güncellendi. | `npx tsc --noEmit` EXIT=0 · `python scratch/test_blastoise_hydro_pump.py` PASS |
