...
5. **Preview Stage vs. In-Game Active Card Dimension & Parity Discipline (Önizleme Sahnesi & Oyun İçi Kart Parite İlkesi):**
   - **Boyutsal Ayrışma Riski (Dimension Desynchronization):** Önizleme (showcase) HTML sayfalarında (`preview_*.html`), test kartı kapsayıcılarının (`.card-stage`) rastgele veya tahmini boyutlarla (örneğin `180px × 252px`) inşa edilmesi; gerçek oyun içi aktif kart boyutlarıyla (`CardView.tsx` `size="active"` = `184px × 253px`, `aspect-ratio: 600 / 825`) mikro-ölçek uyuşmazlığı yaratır.
   ...
   - **Parite Standartları:**
     1. *Piksel-Piksele Boyut Eşitliği:* Tüm bağımsız önizleme sahneleri, oyun içi masaüstü aktif kart boyutları olan `184px × 253px` ölçüsünü ve `aspect-ratio: 600 / 825` oranını birebir kullanmalıdır.
     2. *Otantik Kart Altlığı Gereksinimi (Authentic Underlay Standard):* Önizleme sayfaları, test edilen saldırının hedef kart tipine uygun otantik Base Set / Jungle / Fossil kart görsellerini (`cards/53.jpg` Magnemite, `cards/48.jpg` Grimer vb.) alt katman (`underlay`) olarak barındırmalıdır. Geliştiricinin hem pürüzsüz kart illüstrasyonunu hem de minimalist soyut aşamayı kıyaslayabilmesi için dinamik bir "Mod Değiştirici" (Toggle Mode) düğmesi sunulmalıdır.
     3. *Stil ve Kod Özdeşliği (Zero-Deviation CSS/JSX):* Önizleme HTML dosyasındaki inline JS stilleri ve DOM yapısı ile `BattleFXOverlay.tsx` içindeki üretim JSX yapısı arasında sıfır fark bulunmalıdır; önizlemede çalışan bir koordinat kilitlenmesi üretim kodundan esirgenemez.
